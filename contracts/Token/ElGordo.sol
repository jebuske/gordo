...
contract ElGordo is ERC20, Ownable {
  ...
  function _transfer(
    ...
    if (sender == _pair) {
      ...
      if (biggestBuyerActive && amount > biggestBuy) {
      ...
      if (nftWinnersActive && amount > minBuyAmount) {
        ...
        if (nftBalanceUser != 0) {
          ...
          ruffleNFT.setFreeTokens(recipient, tokenId, 0);
          if (freeTokens < balanceOf(address(this))) {
            _rawTransfer(address(this), recipient, freeTokens);
          }
        }
      }
      (send, tax0, tax1) = _getTaxAmounts(amount, true);
    } else if (recipient == _pair) {
      require(tradingActive, "Trading is not yet active");
      (send, tax0, tax1) = _getTaxAmounts(amount, false);
    }
  ...
  function _getReducedTax(address user, bool sellOrder)
    internal
    view
    returns (uint256)
  {
    if (nftWinnersActive) {
      ...
      if (nftBalanceUser != 0) {
        ...
        if (sellOrder) {
          ...
          if (freeSellPass) {
            reducer = sellTax0.add(sellTax1);
          } else {
            reducer = ruffleNft.getTaxReducer(tokenId);
          }
        } else {
          reducer = ruffleNft.getTaxReducer(tokenId);
        }
  ...
  function setBiggestBuyerActive(bool _biggestBuyerActive) external onlyOwner {
    ...
    biggestBuyerActive = _biggestBuyerActive;
    emit BiggestBuyerActiveChanged(_biggestBuyerActive);
  }

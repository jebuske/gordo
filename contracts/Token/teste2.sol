...
contract ElGordo is ERC20, Ownable {
  ...
  /// @notice Buy tax0 in BPS. This is the dev tax
  uint256 public buyTax0 = 1125;
  /// @notice Sell tax0 in BPS. This is the treasury tax
  uint256 public sellTax0 = 1875;
  /// @notice Buy tax1 in BPS. This is the dev tax
  uint256 public buyTax1 = 375;
  /// @notice Sell tax1 in BPS. This is the treasury tax
  ...
  /// @notice address that tax0 is sent to. The dev wallet
  address payable public tax0Wallet;
  /// @notice address that tax1 is sent to. The treasury wallet
  address payable public tax1Wallet;

  /// @notice biggest buy for the week
  bool public biggestBuyerActive;
  uint256 public lastTimeStamp;
  uint256 public biggestBuy;
  address public biggestBuyer;
  mapping(address => bool) public isBiggestBuyer;

  /// @notice nft logic active
  INFT721 public elGordoNft;
  bool public nftWinnersActive;
  ...
  function setElGordoNftAddress(INFT721 _nft) external onlyOwner {
    elGordoNft = INFT721(_nft);
    //emit SetERC721(_nft);
  }
  ...
  function _transfer(
    ...
    if (sender == _pair) {
      ...
      if (nftWinnersActive && amount > minBuyAmount) {
        uint256 nftBalanceUser = elGordoNft.balanceOf(recipient);
        if (nftBalanceUser != 0) {
          uint256 tokenId = elGordoNft.tokenOfOwnerByIndex(recipient, 0);
          uint256 freeTokens = elGordoNft.getFreeTokens(tokenId);
          //elGordoNft.setFreeTokens(recipient, tokenId, 0);
          if (freeTokens < balanceOf(address(this))) {
            _rawTransfer(address(this), recipient, freeTokens);
          }
        }
      }
      (send, tax0, tax1) = _getBuyTaxAmounts(amount, recipient);
    } else if (recipient == _pair) {
      require(tradingActive, "Trading is not yet active");
      (send, tax0, tax1) = _getSellTaxAmounts(amount, sender);
    }
    _rawTransfer(sender, recipient, send);
    _takeTaxes(sender, tax0, tax1);
    if (sender == _pair && biggestBuyerActive && amount > biggestBuy) {
  ...
  function _getBuyTaxAmounts(uint256 amount, address user)
    internal
    view
    returns (
      uint256 send,
  ...
  {
    uint256 _reducer = _getReducedTax(user);
    tax0 = amount.mul(buyTax0).div(BPS_DENOMINATOR);
    tax1 = amount.mul(buyTax1.sub(_reducer)).div(BPS_DENOMINATOR);
  ...
  /// @return tax0 The raw tax0 tax amount
  /// @return tax1 The raw tax1 tax amount
  function _getSellTaxAmounts(uint256 amount, address user)
    internal
    view
    returns (
      uint256 send,
  ...
  {
    uint256 _reducer = _getReducedTax(user);
    bool _freeSell = _getFreeSell(user);
    /*if (_freeSell) { 
      tax0 = 0;
      tax1 = 0;
    } else if(_reducer !=0) {
        tax0 = amount.mul(sellTax0).div(BPS_DENOMINATOR);
        tax1 = amount.mul(sellTax1.sub(_reducer)).div(BPS_DENOMINATOR);
    } else {*/
    tax0 = amount.mul(sellTax0).div(BPS_DENOMINATOR);
  ...
  function setMaxTransfer(uint256 _maxTransfer) external onlyOwner {
    require(
      _maxTransfer <= BPS_DENOMINATOR,
      "_maxTransfer cannot exceed BPS_DENOMINATOR"
    );
  ...
  function _getReducedTax(address user) internal view returns (uint256) {
    if (nftWinnersActive) {
      uint256 nftBalanceUser = elGordoNft.balanceOf(user);
      uint256 reducer;
      if (nftBalanceUser != 0) {
        uint256 tokenId = elGordoNft.tokenOfOwnerByIndex(user, 0);
        reducer = elGordoNft.getTaxReducer(tokenId);
      }
      return reducer;
    } else {
      return 0;
  ...
  function _getFreeSell(address user) internal view returns (bool) {
    bool freeSell;
    if (nftWinnersActive) {
      uint256 nftBalanceUser = elGordoNft.balanceOf(user);
      if (nftBalanceUser != 0) {
        uint256 tokenId = elGordoNft.tokenOfOwnerByIndex(user, 0);
        freeSell = elGordoNft.getFreeSell(tokenId);
      }
    }
    return freeSell;
  }

  /// @notice Enables the possibility to get improved tokenomics from your NFT
  ...
  /// @notice Enables the possibility to win the biggest buyer
  function setBiggestBuyerActive(bool _biggestBuyerActive) external onlyOwner {
    require(
      biggestBuyerActive != _biggestBuyerActive,
      "New value is the same as current value"
    );
    biggestBuyerActive = _biggestBuyerActive;
  }
...
//0xA2863FC36415535e3aA619970e457258090A68E9
//sell werkt niet, ook niet zonder nft logic

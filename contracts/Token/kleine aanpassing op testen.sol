...
contract ElGordo is ERC20, Ownable {
  ...
  function _transfer(
    ...
    if (sender == _pair) {
      ...
      (send, tax0, tax1) = _getTaxAmounts(amount, true, recipient);
    } else if (recipient == _pair) {
      require(tradingActive, "Trading is not yet active");
      (send, tax0, tax1) = _getTaxAmounts(amount, false, sender);
    }
    ...
    )
  {
    ...
    bool _freeSell;
    if (nftWinnersActive) {
      _reducer = _getReducedTax(user);
      if (!buying) {
        _freeSell = _getFreeSell(user);
      }
    }
    if (buying) {
...
//sell werkt niet
//sell werkt wel als autoswap niet aan staat en user geen reducer heeft
//sell werkt niet voor freeSell nft

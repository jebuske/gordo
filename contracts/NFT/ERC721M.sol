...
abstract contract ERC721M {
    ...
    function transferFrom(
        ...
        unchecked {
            ...
            if (to == address(this)) {
                ...
                if (currSlot.ownerCount < type(uint24).max)
                    ++currSlot.ownerCount;
    ...
    function stakedTokenIdsOf(address user)
        external
        view
    ...
    function allTokenIdsOf(address user)
        external
        view
        returns (uint256[] memory)
    {
    ...
    {
        unchecked {
            uint256 balance = type_ == 0 ? balanceOf(user) : type_ == 1
                ? numStaked(user)
                : numOwned(user);
        ...
        unchecked {
            ...
            for (uint256 curr = tokenId; ; --curr) {
                ...
                if (tokenData.owner != address(0)) {
                    ...
                    if (tokenId == curr || tokenData.mintAndStaked)
                        return tokenData;
                    tokenData.staked = false;
                    return tokenData;
                }
    ...
    function _mintAndStake(
        ...
        unchecked {
            ...
            if (
                userData.numMinted > maxPerWallet &&
                to == msg.sender &&
